...
class timeseries_test():
  ...
  def __call__(self, df_test):
    """
    with open(config_yaml, "r") as f:
      global_config= yaml.load(f, Loader=yaml.FullLoader)
    csv_path = os.getcwd()+global_config["csv_path"]
    """
    self.dataLoader = dataLoader("","pm100")
    X_features = ['node_count','cores_per_task', 'num_cores_req', 'num_nodes_req', 'num_tasks', 'priority', 'num_gpus_req','mem_req']
    processed_test_data = df_test[X_features]

    clusters, probabilities = ClusterClassifier._predict_clusters(self.dataLoader, processed_test_data) 
    ...
    for i in range(5):
        ...
        model.load_state_dict(torch.load(f"/work/08389/hcs77/ls6/power-prediction/models/time_series_models/TimeSeriesModel-{i}.pth"))
        #model.load_state_dict(torch.load(os.getcwd()+self.dataLoader.config.get('datasets',{}).get('pm100',{}).get("time_series_models")+f"TimeSeriesModel-{i}.pth")) #"/work/08389/hcs77/ls6/application-fingerprinting/Models/rnn_model.pth"))
    ...
    print(predictions)
    """
    X_static_tensor = torch.tensor(processed_test_data.values , dtype=torch.float32)
    #print("loaded_df column names")
    #print(loaded_df.columns)
    print("X Tensor shape")
    print(X_static_tensor.shape)
    #test_labels = util.find_nearest_centroids(X_static_tensor, torch.tensor(centroids, dtype=torch.float32))
    #print(test_labels)
    predictions =[]
    for i in range(len(X_static_tensor)):
        #folder_name = f"{rank}"
        #print(f"************Rank {rank} is entering *******")
        selected_model = models[clusters[i]]
        y_pred = selected_model(X_static_tensor[i])
        print("X_static tensor")
        print(X_static_tensor[i])
        print("Predictions")
