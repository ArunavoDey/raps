...
# === CONFIGURATION ===
plots = ['power', 'stats']  # Power and stats only for the final plot
plot_files = {
...
print(week_to_policies)
num_weeks = len(weeks)

# === SPECIFICATION ===
# Define the specs as 2D list, one row, with num_weeks columns
# The first n-1 columns for power (empty) and last column for radar chart (polar)
...
specs.append({"type": "polar"})  # Last column is for the radar chart
print([specs])

# Create subplot grid with correct specs
fig = sp.make_subplots(
    rows=1, cols=num_weeks,  # num_weeks power plots + 1 radar chart column
    specs=[specs],
...
for i, (week, policy_dirs) in enumerate(week_to_policies.items()):
    ...
    show_legend = i == 0  # Only show legend on first week for the first plot type
    if i < num_weeks - 1:
    ...
    if i == num_weeks - 1:  # Only add radar chart in the last column
        all_stats = {}
        for policy_dir in policy_dirs:
        ...
        for policy, stats in normalized_stats.items():
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                fill='toself',
                name=policy,
                subplot=f"polar1",  # Correctly reference the polar chart
...
for col in range(1, num_weeks + 1):
    if col < num_weeks:  # Last column is for radar chart
        ...
        fig.update_xaxes(
            title_text="Time [days]" if plots[0] == 'power' else "",
            row=1, col=col, showline=True,
            linecolor='black',
            showgrid=True,
            gridcolor='lightgray'
        )
...
# === SAVE ===
#fig.write_image("final_power_and_stats_plot.pdf", width=1000 * (num_weeks), height=2000)
fig.write_image("final_power_and_stats_plot.pdf")

print("Plot saved to: final_power_and_stats_plot.pdf")
